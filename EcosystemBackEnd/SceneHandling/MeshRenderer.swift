...
////                let vertex = SCNVector3(x: width*CGFloat(w)/CGFloat(widthCount)-width/2, y: 2 * CGFloat(sin(Float(w) * 2 * 5 * Float.pi / Float(widthCount)) + 2 * cos(Float(h)*2*7*Float.pi/Float(heightCount))), z: height*CGFloat(h)/CGFloat(heightCount)-height/2)
